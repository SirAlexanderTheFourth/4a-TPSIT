...
void confronto(Elemento *head_coda, Elemento *tail_coda, Elemento *head_pila){
    ...
    printf("elemto coda n%d, valore %d\n", confronto_c->numero_arrivo, confronto_c->valore);
    printf("elemto pila n%d, valore %d\n", confronto_p->numero_arrivo, confronto_p->valore);
    if(confronto_c->valore>confronto_p->valore){
        confronto_c->valore -= confronto_p->valore;
        printf("\nVincitore: elemento coda\nnuovo valore: %d\n", confronto_c->valore);
        enqueue(&head_coda, &tail_coda, confronto_c);
        free(confronto_p);
        
    }else if(confronto_p->valore>confronto_c->valore){
        printf("\nVincitore: elemento pila\nnuovo valore: %d\n", confronto_p->valore);
...
int main(){
    ...
    while(current_head->next==NULL || head_pila->next==NULL){
        printf("Coda:\n");
        stampa(current_head);
